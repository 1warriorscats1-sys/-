...
// TH095 port: the reconstruction's C_ASSERT checks document the 32-bit i386
// target layout and cannot hold on 64-bit platforms. Compiled out here; the
// strict-match DIFFBUILD path (upstream VC7.1 toolchain) uses the real check.
#define C_ASSERT(e) static_assert(true, "th095 port: 32-bit layout assert disabled")
