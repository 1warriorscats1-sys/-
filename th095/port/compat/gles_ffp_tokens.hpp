// gles_ffp_tokens.hpp — desktop-GL (fixed-function pipeline) token values
// that GLES3/gl3.h does not define. Kept in a separate header so both the
// shim (gles_ffp.cpp) and d3d8_compat.cpp see them. Tokens already defined
// by gl3.h (GL_REPLACE for stencil, GL_LINEAR, ...) are not redefined.
//
// Modeled on the TH08 port's gles_ffp_tokens.hpp (N0zoM1z0/th08, MIT).
#pragma once
