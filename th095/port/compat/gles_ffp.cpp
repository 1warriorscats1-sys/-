// gles_ffp.cpp — fixed-function shim implementation on top of GLES 3.
//
// Architecture (from the TH08 port report + audit fixes, N0zoM1z0/th08, MIT):
//  • One shader for everything: uMvp * vec4(pos,1); texture sources
//    (0=texture, 1=vertex color, 2=constant TFACTOR), replace/modulate ops
//    for RGB and alpha separately, alpha-test discard, linear fog
//    mix(clamp((end-coord)/(end-start))).
//  • Vertices are accumulated in glBegin/glEnd into a 28-byte struct array;
//    at glEnd — glBufferData(STREAM_DRAW) + glVertexAttribPointer x4 +
//    glDrawArrays.
//  • Own column-major float[16] matrices, stacks of depth 32, glOrtho
//    post-multiplies, MVP = P*MV.
//  • Audit fixes carried over: dirty flags on uniforms (uMvp only changes
//    when matrices change), glUseProgram never deactivates, attribute
//    arrays enabled once.
#include "gles_ffp.hpp"

// IMPORTANT: inside the ffp:: implementation the shim macros must be
// removed — otherwise ::glEnable/::glDisable/::glDepthMask/
// ::glTexParameteri in function bodies would expand to ffp::Enable/...
// (infinite recursion). The macros are needed by the shim CONSUMERS
// (d3d8_compat.cpp) only.
#undef glBegin
#undef glEnd
#undef glVertex2f
#undef glVertex3f
...
// Sources for the combiner: 0=texture, 1=vertex color, 2=constant.
enum CombineSource { SOURCE_TEXTURE = 0, SOURCE_VERTEX = 1, SOURCE_CONSTANT = 2 };
// Ops: 0=replace (source0 only), 1=modulate (source0*source1).
...
GLuint CompileShader(GLenum kind, const char *source)
{
    ...
    if (compiled == GL_FALSE)
    {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "th095-port: shader compile failed: %.*s",
...
bool EnsureProgram()
{
    ...
    if (linked == GL_FALSE)
    {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "th095-port: program link failed: %.*s",
                     static_cast<int>(length), log);
        return false;
    }
    ...
    // The program is activated ONCE and never deactivated.
    ::glUseProgram(program);
    ::glUniform1i(g_locTexture, 0);

    // Attribute arrays are enabled once.
...
int CombineSourceFromGl(GLenum source)
{
    ...
    default: return SOURCE_VERTEX; // GL_PRIMARY_COLOR, GL_PREVIOUS, ...
    }
}
...
void End()
{
    ...
    // uMvp is re-uploaded only when the matrices change.
    if (g_mvpDirty)
    {
           ...
           GLdouble nearVal, GLdouble farVal)
{
    ...
    ortho[15] = 1.0f;
    // glOrtho POST-multiplies the current matrix.
    float product[16];
...
void PopAttrib()
{
    ...
    // Re-apply the real caps from the snapshot.
...
void TexEnvi(GLenum target, GLenum pname, GLint param)
{
    ...
    case GL_TEXTURE_ENV_MODE:
        // MODE=REPLACE -> (texture, replace); MODULATE -> texture*vertex color;
        // COMBINE -> keep the configured combined sources.
        if (param == GL_REPLACE)
        {
    ...
    case GL_SOURCE1_ALPHA:
        ...
        break;
    default:
        break; // OPERAND0/1 — always SRC_COLOR/SRC_ALPHA from d3d8_compat
    }
    g_texEnvDirty = true;
}
...
{
    // Only linear fog by fog coordinate is supported (what d3d8_compat
    // sends): GL_FOG_MODE/GL_FOG_COORDINATE_SOURCE have no effect.
...
void SwitchPace()
{
    // 60 Hz pacer on an absolute deadline: eglSwapBuffers on Horizon/Mesa
    // does NOT block; without a pacer the game runs free.
    ...
    if (deadlineBase == 0)
        deadlineBase = now;
    // Re-sync when falling behind by more than 4 frames.
