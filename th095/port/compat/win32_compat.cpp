...
void SetApplicationIcon(SDL_Window *window)
{
    if (window == NULL)
        return;
    const std::string iconPath = ExecutableSiblingPath("th095-port.png");
    if (iconPath.empty())
        return;
...
HWND CreateWindowExA(DWORD, LPCSTR, LPCSTR title, DWORD style, int, int, int width, int height, HWND, HANDLE, HINSTANCE, LPVOID)
{
    ...
    if (g_window == NULL) fprintf(stderr, "th095-port: SDL_CreateWindow failed: %s\n", SDL_GetError());
