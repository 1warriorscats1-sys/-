...
class LinuxDevice : public IDirect3DDevice8
{
  public:
    ...
    HRESULT BeginScene()
    {
        // TH08 froze the background behind dialogues by snapshotting it here.
        // TH095's GUI state lives in different structs; the snapshot hook is
        // intentionally a no-op for now (dialogues render over live scenes).
        return S_OK;
    }
    HRESULT EndScene() { return S_OK; }
    HRESULT Clear(DWORD, const D3DRECT *, DWORD flags, D3DCOLOR color, float depth, DWORD)
    {
        if ((flags & D3DCLEAR_TARGET) && getenv("TH095_RENDER_TRACE") != NULL)
        {
  ...
  private:
    ...
    bool CreateRenderTarget(UINT width, UINT height)
    {
        ...
        {
            fprintf(stderr, "th095-port: OpenGL framebuffer objects are unavailable\n");
            return false;
        }
        ...
        {
            fprintf(stderr, "th095-port: unable to create OpenGL framebuffer (status 0x%04x)\n",
