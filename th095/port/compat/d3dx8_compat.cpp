                          ...
                          const RECT *sourceRectRaw, D3DCOLOR colorKey)
{
    ...
    if (!th095_linux_surface_access(destinationRaw, &destination, false) || destination.pixels == NULL)
        return E_INVALIDARG;
...
// supported D3DFORMAT) into the destination surface with scaling, matching
// the D3DX semantics the game relies on for textures and photo shots.
HRESULT D3DXLoadSurfaceFromMemory(IDirect3DSurface8 *destinationRaw, const RECT *,
                                  const RECT *destinationRectRaw, const void *memory,
                                  D3DFORMAT sourceFormat, int sourcePitch, const RECT *,
                                  const RECT *sourceRectRaw, DWORD, D3DCOLOR colorKey)
{
    LinuxSurfaceAccess destination;
    if (memory == NULL) return E_INVALIDARG;
    if (!th095_linux_surface_access(destinationRaw, &destination, false) || destination.pixels == NULL)
        return E_INVALIDARG;
    if (sourceFormat == D3DFMT_UNKNOWN) sourceFormat = destination.format;
    const BYTE *source = static_cast<const BYTE *>(memory);
    UINT sourceBytes = BytesPerPixel(sourceFormat);
    if (sourceBytes == 0) return E_INVALIDARG;
    UINT sourceWidth = static_cast<UINT>(sourcePitch) / sourceBytes;

    RECT destinationRect = destinationRectRaw != NULL ? *destinationRectRaw
                                                      : FullRect(destination.width, destination.height);
    RECT sourceRect = sourceRectRaw != NULL ? *sourceRectRaw : FullRect(sourceWidth, 0);
    if (!ValidRect(destinationRect)) return E_INVALIDARG;
    if (sourceRect.top < 0 || sourceRect.left < 0) return E_INVALIDARG;
    if (destinationRect.right > static_cast<LONG>(destination.width)) destinationRect.right = destination.width;
    if (destinationRect.bottom > static_cast<LONG>(destination.height)) destinationRect.bottom = destination.height;
    if (sourceRect.right > static_cast<LONG>(sourceWidth)) sourceRect.right = sourceWidth;
    UINT destinationWidth = destinationRect.right - destinationRect.left;
    ...
    UINT sourceRectWidth = sourceRect.right - sourceRect.left;
    UINT sourceRectHeight = sourceRect.bottom - sourceRect.top;
    if (sourceRectHeight == 0) sourceRectHeight = 1;
    UINT destinationBytes = BytesPerPixel(destination.format);
    for (UINT y = 0; y < destinationHeight; ++y)
    {
        UINT sourceY = sourceRect.top + static_cast<UINT>((static_cast<unsigned long long>(y) * sourceRectHeight) / destinationHeight);
        for (UINT x = 0; x < destinationWidth; ++x)
        {
            UINT sourceX = sourceRect.left + static_cast<UINT>((static_cast<unsigned long long>(x) * sourceRectWidth) / destinationWidth);
            BYTE rgba[4];
            DecodePixel(source + sourceY * static_cast<UINT>(sourcePitch) + sourceX * sourceBytes, sourceFormat, rgba);
            if (colorKey != 0 && (colorKey & 0x00ffffffu) ==
                                 ((static_cast<DWORD>(rgba[0]) << 16) | (static_cast<DWORD>(rgba[1]) << 8) | rgba[2]))
                rgba[3] = 0;
            EncodePixel(destination.pixels + (destinationRect.top + y) * destination.pitch +
...
// Added for the TH095 port (D3DXVec4Transform, no perspective division —
// same row-vector convention as D3DXVec3TransformCoord above).
void D3DXVec4Transform(D3DXVECTOR4 *out, const D3DXVECTOR4 *value, const D3DXMATRIX *matrix)
{
    if (out == NULL || value == NULL || matrix == NULL)
        return;
    out->x = value->x * matrix->_11 + value->y * matrix->_21 + value->z * matrix->_31 + value->w * matrix->_41;
    out->y = value->x * matrix->_12 + value->y * matrix->_22 + value->z * matrix->_32 + value->w * matrix->_42;
    out->z = value->x * matrix->_13 + value->y * matrix->_23 + value->z * matrix->_33 + value->w * matrix->_43;
    out->w = value->x * matrix->_14 + value->y * matrix->_24 + value->z * matrix->_34 + value->w * matrix->_44;
}
