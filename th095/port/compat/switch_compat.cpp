// switch_compat.cpp — Win32-subset on top of SDL2 + libnx for the Nintendo
// Switch (the Switch twin of win32_compat.cpp).
//
// Differences from win32_compat.cpp (following the TH08 port's
// switch_compat.cpp, N0zoM1z0/th08, MIT):
//  * SD data-directory search: roots (sdmc:/switch, sdmc:/, sdmc:/games,
//    sdmc:/roms) x folder names; first one with th095.dat, else the first
//    existing one, else sdmc:/switch/th095.
//  * TranslatePath(): relative paths -> <data dir>/path (with the virtual
//    cwd from _chdir taken into account); absolute paths pass through.
//  * FindFirstFileA: opendir/readdir + own case-insensitive '?*' matching.
//  * CP932 -> Unicode via the generated table (no iconv in newlib).
//  * The font is looked up in the data directory (no fontconfig).
//  * Input from the gamepad (stick dead zone 8000), filling BOTH the VK
//    and the DIK tables.
//  * Window 1280x720, GLES 3 (ES profile, major 3).
//  * Timing on clock_gettime(CLOCK_MONOTONIC).
//  * Audio: SDL callback 44100/S16/stereo, samples 1024.
//  * GetStartupInfoA reports lpTitle as the (existing) th095.dat in the
//    data dir, different from GetModuleFileNameA — the game then sets
//    usesRelativePath -> disableVsync and skips the boot FPS benchmark
//    (Supervisor::CheckFps), the legal "relative launch" semantics built
//    into the game itself.
//  * CreateThread gives threads an honest 1 MiB stack (libnx's default
//    pthread stack is small; the game always passes dwStackSize=0).
...
// ---------------------------------------------------------------------------
// Data directory and path translation
// ---------------------------------------------------------------------------

const char *kSdRoots[] = { "sdmc:/switch", "sdmc:/", "sdmc:/games", "sdmc:/roms" };
const char *kFolderNames[] = {
    "th095", "touhou95", "touhou 95", "th095-switch", "touhou95-switch",
    "shoot the bullet", "th09.5", "in",
};

std::string g_dataDir;
bool g_dataDirResolved;
std::string g_virtualCwd; // "" or e.g. "backup"/"replay"
...
void ResolveDataDir()
{
    ...
    for (size_t root = 0; root < sizeof(kSdRoots) / sizeof(kSdRoots[0]); ++root)
    {
        for (size_t name = 0; name < sizeof(kFolderNames) / sizeof(kFolderNames[0]); ++name)
        {
            ...
            if (PathExists(candidate + "/th095.dat", false))
            {
                g_dataDir = candidate;
                return;
            }
    ...
    g_dataDir = firstExisting.empty() ? "sdmc:/switch/th095" : firstExisting;
}
...
// All relative paths (including "./thbgm.dat") are translated to
// <data dir>/[<virtual cwd>/]path; absolute paths pass through.
...
int SetVirtualCwd(const char *path)
{
    ...
    if (IsAbsolutePath(cleaned))
    {
        // Treat sdmc:/... relative to the data dir.
        ResolveDataDir();
...
// ---------------------------------------------------------------------------
// CP932 -> Unicode/UTF-8 (no iconv in newlib; generated table)
// ---------------------------------------------------------------------------

unsigned short LookupCp932(unsigned short code)
{
    const th095_switch::Cp932Entry *table = th095_switch::g_Cp932Table;
    int low = 0, high = th095_switch::g_Cp932TableSize - 1;
    while (low <= high)
    {
...
// Wide -> narrow ASCII (the game only passes ASCII paths to the W APIs).
std::string WCharToAscii(const wchar_t *text)
{
    std::string result;
    if (text == NULL)
        return result;
    for (size_t index = 0; text[index] != 0; ++index)
    {
        ...
        result += static_cast<char>(value < 0x100 ? value : '?');
    }
    return result;
}

// ---------------------------------------------------------------------------
// Font (no fontconfig: msgothic.ttc -> meiryo -> any *.ttf/*.ttc in data)
// ---------------------------------------------------------------------------
...
const char *ResolveJapaneseFont()
{
    ...
    const char *overridePath = getenv("TH095_FONT");
    ...
    ResolveDataDir();
    const char *preferred[] = { "msgothic.ttc", "msgothic.ttf" };
...
{
    // pthread_t in newlib is a pointer.
...
{
    // '?' — exactly one character, '*' — any sequence, case-insensitive.
    if (*pattern == '\0')
        return *name == '\0';
    if (*pattern == '*')
    {
...
// Fills BOTH the VK table (GetKeyboardState) and the DIK table
// (DirectInput): the game walks both routes.
...
#define SET_KEY(code) state[(code)] = 0x80
    if (buttons.up) { SET_KEY(VK_UP); SET_KEY(DIK_UP); SET_KEY(DIK_NUMPAD8); }
    if (buttons.down) { SET_KEY(VK_DOWN); SET_KEY(DIK_DOWN); SET_KEY(DIK_NUMPAD2); }
    if (buttons.left) { SET_KEY(VK_LEFT); SET_KEY(DIK_LEFT); SET_KEY(DIK_NUMPAD4); }
    if (buttons.right) { SET_KEY(VK_RIGHT); SET_KEY(DIK_RIGHT); SET_KEY(DIK_NUMPAD6); }
    if (buttons.shot) { SET_KEY('Z'); SET_KEY(DIK_Z); }
    if (buttons.bomb) { SET_KEY('X'); SET_KEY(DIK_X); }
    if (buttons.focus) { SET_KEY('P'); SET_KEY(DIK_P); SET_KEY(VK_SHIFT); SET_KEY(DIK_LSHIFT); SET_KEY(DIK_RSHIFT); }
    if (buttons.skip) { SET_KEY(VK_RETURN); SET_KEY(DIK_RETURN); SET_KEY(VK_CONTROL); SET_KEY(DIK_LCONTROL); SET_KEY(DIK_RCONTROL); }
    if (buttons.menu) { SET_KEY(VK_ESCAPE); SET_KEY(DIK_ESCAPE); }
#undef SET_KEY
    (void)directInput;
}
} // namespace
...
extern "C" const char *th095_switch_data_dir() { ResolveDataDir(); return g_dataDir.c_str(); }
...
HANDLE CreateFileW(LPCWSTR path, DWORD access, DWORD share, LPVOID security, DWORD disposition, DWORD attrs, HANDLE templ)
{
    ...
    return CreateFileA(WCharToAscii(path).c_str(), access, share, security, disposition, attrs, templ);
}
...
DWORD GetFileAttributesW(LPCWSTR path)
{
    ...
    std::string translated = TranslatePath(WCharToAscii(path).c_str());
    struct stat info;
...
BOOL SetCurrentDirectoryW(LPCWSTR path)
{
    ...
    return SetVirtualCwd(WCharToAscii(path).c_str()) == 0;
}
...
HANDLE CreateThread(LPVOID, size_t, LPTHREAD_START_ROUTINE start, LPVOID parameter, DWORD, LPDWORD id)
{
    ...
    // The game always passes dwStackSize=0 ("default"), and libnx's default
    // pthread stack is small — give an honest megabyte.
...
// MSVC thread entry points used by the reconstruction. _beginthreadex returns
// a ThreadHandle so the game's CloseHandle() releases it exactly like on
// Windows; _beginthread is fire-and-forget (detached).
namespace {
struct RawThreadState
{
    RawThreadState(void (*start_)(void *), void *arg_) : start(start_), arg(arg_) {}
    ~RawThreadState() { start(arg); }
    void (*start)(void *);
    void *arg;
};
void *RawThreadTrampoline(void *opaque) { delete static_cast<RawThreadState *>(opaque); return NULL; }
} // namespace

unsigned _beginthread(void (*start)(void *), unsigned, unsigned, void *arg)
{
    RawThreadState *state = NULL;
    try { state = new RawThreadState(start, arg); } catch (...) { return static_cast<unsigned>(-1); }
    pthread_t thread;
    if (pthread_create(&thread, NULL, RawThreadTrampoline, state) != 0) { delete state; return static_cast<unsigned>(-1); }
    pthread_detach(thread);
    return 0;
}

unsigned _beginthreadex(void *, unsigned, unsigned (__cdecl *start)(void *), void *arg, unsigned, unsigned *id)
{
    ThreadHandle *handle = new ThreadHandle();
    handle->start = reinterpret_cast<LPTHREAD_START_ROUTINE>(start);
    handle->parameter = arg;
    if (pthread_create(&handle->thread, NULL, ThreadTrampoline, handle) != 0) { delete handle; return 0; }
    while (handle->id == 0) usleep(100);
    if (id != NULL) *id = handle->id;
    return reinterpret_cast<unsigned>(handle);
}

void _endthread(void) {}
unsigned _endthreadex(unsigned code) { return code; }
...
HWND CreateWindowExA(DWORD, LPCSTR, LPCSTR title, DWORD, int, int, int, int, HWND, HANDLE, HINSTANCE, LPVOID)
{
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER) != 0)
    { fprintf(stderr, "th095-port: SDL_Init failed: %s\n", SDL_GetError()); return NULL; }
    // Context attributes BEFORE the window: GLES 3 (ES profile). The
    // devkitPro SDL2 backend requests EGL_OPENGL_ES3_BIT for major=3.
    ...
    g_window = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, flags);
    if (g_window == NULL)
        fprintf(stderr, "th095-port: SDL_CreateWindow failed: %s\n", SDL_GetError());
    else
        OpenGamepad();
...
int ShowCursor(BOOL) { return SDL_ShowCursor(SDL_DISABLE); }
...
{
    if (title != NULL) fprintf(stderr, "%s: ", WCharToAscii(title).c_str());
    if (text != NULL) fprintf(stderr, "%s\n", WCharToAscii(text).c_str());
    return 0;
}
...
{
    // lpTitle points at an EXISTING file (with an extension) that differs
    // from what GetModuleFileNameA returns (the data dir). The game then
    // sets usesRelativePath -> disableVsync and skips the boot FPS benchmark
    // (Supervisor::CheckFps) entirely, with dummyMidiTimerEnabled=false —
    // the same "relative launch" semantics it has when started from the
    // Windows Explorer.
    ...
    value->lpTitle = title[0] != 0 ? title : NULL;
}
...
HMODULE LoadLibraryA(LPCSTR) { return NULL; } // no dlopen in newlib
...
HFONT CreateFontA(int height, int, int, int, int weight, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, LPCSTR)
{
    ...
    {
        fprintf(stderr, "th095-port: SDL_ttf initialization failed: %s\n", TTF_GetError());
        return new GdiFont();
    }
    TTF_Font *font = TTF_OpenFont(path, height < 0 ? -height : height);
    if (font == NULL)
    {
        fprintf(stderr, "th095-port: unable to load Japanese font %s: %s\n", path, TTF_GetError());
        return new GdiFont();
    }
...
const DIDATAFORMAT c_dfDIJoystick2 = {sizeof(DIDATAFORMAT)}; // th095 port
const GUID DI8DEVCLASS_GAMECTRL = {0x6f1d2b61, 0xd300, 0x11cf, {0xbf, 0xcf, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00}}; // th095 port
...
class SwitchDirectInput : public IDirectInput8A
{
  public:
    ...
    HRESULT EnumDevices(const GUID, LPDIENUMDEVICESCALLBACKA, LPVOID, DWORD) { return S_OK; }
  private: ULONG refs;
};
...
void EnsureAudio()
{
    ...
    { fprintf(stderr, "th095-port: SDL audio initialization failed: %s\n", SDL_GetError()); return; }
    ...
    if (g_audioDevice == 0)
    { fprintf(stderr, "th095-port: SDL audio device unavailable: %s\n", SDL_GetError()); return; }
