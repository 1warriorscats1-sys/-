/* Original WWW compatibility code. MIT; see the toolkit LICENSE.md.
...
static int www_save_exists(const char *path) {
    struct stat st;
    return stat(path, &st) == 0 && S_ISREG(st.st_mode);
}

/* If replacement was interrupted after the old file moved, restore it. */
static void www_save_recover(const char *path) {
    size_t n;
    char *backup;
    if (www_save_exists(path)) return;
    n = strlen(path); backup = (char *)malloc(n + 5);
    if (!backup) return;
    memcpy(backup, path, n); memcpy(backup+n, ".bak", 5);
    if (www_save_exists(backup)) (void)rename(backup, path);
    free(backup);
}

static int www_save_text(const char *path, const char *text) {
    ...
    if (www_save_exists(path)) {
        if (www_save_exists(backup) && remove(backup) != 0) goto done;
